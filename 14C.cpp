...
int main()
{
    ...
    if (A == B || C == D) {
        ...
        if (C == D) {
            ...
            if ((D - C).prod(A - C) == 0.0 && isOrdered(C, A, D)) {
                std::cout << "YES\n";
            } else {
                std::cout << "NO\n";
            }
    ...
    if (k.isParallelWith(m)) {
        if (isZero(k.distanceTo(m)) && (isOrdered(A, C, B) || isOrdered(A, D, B) || isOrdered(C, A, D) || isOrdered(C, B, D))) {
            std::cout << "YES\n";
        } else {
            std::cout << "NO\n";
        }
        ...
        if ((isOrdered(A.x, X.x, B.x) && isOrdered(A.y, X.y, B.y))
